...
func _ready_labels():
	...
	for label in level_labels:
		label.visible = false
	if 12 in SavedLevelInfo.solved_levels:
		$GuideLabel.text = "< TO CREDITS"
...
func _ready() -> void:
	...
	while len(used) > 0:
		...
		if SavedLevelInfo.did_just_quit_level == [true, level_num_tag]:
			is_done = true
			player.bounds = group
			player.position = player.bounds.position * grid.tile_set.tile_size
			player.scale = player.bounds.size
			level_labels[level_num_tag - 1].visible = true
...
func _process(_delta: float) -> void:
	if player.position.y == 0 and is_main and not disabled:
		disabled = true
